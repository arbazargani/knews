    ...
    <div class="page-toolbar">
        <div class="btn-group pull-right">
            ...
            <ul class="dropdown-menu pull-right" role="menu">
                <li>
                    <a href="{{ route('admin.cat.create', [$module_name]  ) }}"> <i class="icon-shield"></i> ثبت دسته
                        بندی جدید</a>
                </li>
                <li class="divider"></li>
                <li>
...
<script>
    $(function () {
        var dg = $('#grid1').datagrid({
            ...
            sortOrder: 'asc',
            url: '{{ route('admin.cat.list' , [$module_name]) }}',
            columns: [
                [
            ...
            toolbar: [

                {
                    ...
                    handler: function () {
                        ...
                        if (row) {
                            window.location.href = '{{ url('admin/cat')  }}/' + row.id + '/edit/' + '{{ $module_name }}';
                        }
                        else {
                    ...
                    handler: function () {
                        ...
                        $.messager.confirm('توجه', 'آیا از تغییر وضعیت مطمئن هستید؟', function (r) {
                            if (r) {
                                ...
                                $.post('{{ url('admin/cat')  }}/' + row.id + '/status/'+ '{{ $module_name }}', params, function (data) {
                                    if (data.ok)
                                        dg.datagrid('reload');
                                    else
                    ...
                    handler: function () {
                        ...
                        $.messager.confirm('توجه', 'آیا از حذف مطمئن هستید؟', function (r) {
                            if (r) {
                                ...
                                $.post('{{ url('admin/cat')  }}/' + row.id + '/remove/'+ '{{ $module_name }}', params, function (data) {
                                    if (data.ok)
                                        dg.datagrid('reload');
                                    else
                                        $.messager.alert('توجه', data.message, 'error');
