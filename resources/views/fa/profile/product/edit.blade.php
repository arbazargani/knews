...
@section('title', trans('custom.edit_product') )
            ...
            <div class="form-group">
                <article class="article-info">
                    <div class="article-title">
                        <h1>@lang('custom.edit_product')</h1>
                    </div>

                    <form action="{{ route('profile.products.edit.post',['id'=>$product->id]) }}" id="frm1" method="post" enctype="multipart/form-data" class="form-horizontal">
                        <fieldset id="account">
                            ...
                            <div class="row">
                                @include('partials.text_small' , ['field_title' => trans('custom.product_name') , 'field_name' => 'product_name','star'=>true,'data'=> $product->product_name ])

                                @include('partials.text_small' , ['field_title' => trans('custom.product_model') , 'field_name' => 'product_model','star'=>true,'data'=> $product->product_model])
                            </div>
                            ...
                            <div class="row">
                                @include('partials.text_small' , ['field_title' => trans('custom.minimum_order') , 'field_name' => 'minimum_order','star'=>true ,'data'=> $product->minimum_order])
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    {!! Form::text('supply_ability', $product->supply_ability ,['class' => 'form-control','placeholder' => trans('custom.supply_ability') ,'id' => 'supply_ability' ]) !!}
                                </div>
                                <div class="col-sm-3">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    {!! Form::text('prices', $product->prices ,['class' => 'form-control','placeholder' => trans('custom.prices') ,'id' => 'prices' ]) !!}
                                </div>
                                <div class="col-sm-3">
                            ...
                            <div class="row">
                                @include('partials.text_small' , ['field_title' => trans('custom.delivery_time') , 'field_name' => 'delivery_time','star'=>true ,'data'=>$product->delivery_time ])
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-10">
                                    <select name="product_keyword[]" id="product_keyword" class="form-control"  multiple="multiple">
                                        @foreach(explode(',',$product->product_keyword ) as $key=>$val)
                            ...
                            <div class="row">
                                ...
                                <textarea class="ckeditor form-control" name="packing_details" id="packing_details" rows="6">{{ $product->packing_details }}</textarea>
                            </div>
                            <hr/>
                            <div class="row">
                                <label class="control-label" for="input_text"><span class="text-danger">*</span> @lang('custom.description_detailed'):</label>
                                <textarea class="ckeditor form-control" name="description_detailed" id="description_detailed" rows="6">{{ $product->description_detailed }}</textarea>
                            </div>
                            <hr/>
                        ...
                        <div class="buttons">
                            <div class="pull-right">
                                ...
                                <input type="submit" value="@lang('custom.edit_product')" class="btn btn-primary btnFromSave">
                            </div>
                        </div>
                    </form>
                </article>
...
@endpush
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#brand').val('{{ $product->company_id }}');
            $('#minimum_order_unit').val('{{ $product->minimum_order_unit }}');
            $('#supply_ability_unit').val('{{ $product->supply_ability_unit }}');
            $('#supply_ability_per').val('{{ $product->supply_ability_per }}');
            $('#prices_currency').val('{{ $product->prices_currency }}');
            $('#prices_unit').val('{{ $product->prices_unit }}');
            $('#delivery_terms\\[\\]').val('{{ $product->delivery_terms }}');
            $('#payments_terms\\[\\]').val('{{ $product->payments_terms }}');
            $('#lang').val('{{ $product->lang }}');
            $('#select_cat').val('{{ $product->cat_id }}');
            @php
            $p = \App\ProductCat::find($product->cat_id);
            $pp = $p->parent()->get();
            $v = $pp[0]->parent()->get()[0]->title.' / ' . $pp[0]->title .' / '.$p->title ;
            @endphp
            $('.select_cat').text( '{{ $v }}' );

            @foreach(json_decode($product->file_url , true) as $key=>$val)
