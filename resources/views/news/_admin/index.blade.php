	...
	<div class="page-toolbar">
		<div class="btn-group pull-right">
			...
			<ul class="dropdown-menu pull-right" role="menu">
				...
				<li class="divider"></li>
				<li>
					<a href="{{ route('admin.news.index') }}"> <i class="icon-shield"></i> لیست خبر ها من</a>
				</li>
...
@section('content')
	<h3 class="page-title"> لیست جامع خبر ها:
		<small>تمام خبر ها</small>
	</h3>
            ...
            toolbar: [

                {
                    ...
                    handler: function () {
                        ...
                        if (row) {
                            window.location.href = '{{ route('admin.news.index') }}/' + row.id + '/edit';
                        }
                        else {
                        ...
                        if (row) {
                            $.messager.confirm('توجه', 'آیااز تغییر وضعیت مطمئن هستید؟', function(r){
                                if(r)
                                {
                                    ...
                                    $.post('{{ route('admin.news.deactive') }}', params, function(data){
                                        if( data.ok )
                                            dg.datagrid('reload');
                                        else
                ...
                }, '-',
                {
                    ...
                    handler: function () {
                        var row = $('#grid1').datagrid('getSelected');
                        if (row) {
                            $.messager.confirm('توجه', 'آیا از حرف خبر  مطمئن هستید؟', function(r){
                                if(r)
                                {
                                    var params = {
                                        _method: 'DELETE',
                                    ...
                                    $.post('{{ route('admin.news.delete') }}', params, function(data){
                                        if( data.ok )
                                            dg.datagrid('reload');
                                        else
                                            $.messager.alert('توجه', data.message, 'error');
