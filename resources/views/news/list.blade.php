                ...
                <section class="content">
                    ...
                    @foreach($news as $key=>$val)
                        <article class="list-item">
                            <a target="_blank" href="{{ route( 'news.show',[ isset($subdomain)?($subdomain.'.'.$domain):$domain , $val->_id , $val->title ]) }} ">
                                <div class="col-xs-3 image-container vcenter ">
                                    <figure>
                                        <img class="img-responsive " alt="{{ $val->title }}" src="{{ ($val->image_url == '') ? image_url('/images/no_image.jpg',170,118) : image_url( $val->user_id.'/'. $val->image_url,170,118) }}">
                                    </figure>
                                </div>
                                <div class="col-xs-9 text-container vcenter ">
                                    <h2 class="over-title ">
                                        <time><i class="fa fa-clock-o"></i>
                                            {{  \jDate::forge( $val->created_at )->format('%d %B %Y - %H:m') }}
                                        </time>
                                        {{ $val->pre_title }}
                                    </h2>
